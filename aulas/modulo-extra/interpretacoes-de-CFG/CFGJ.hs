...
case_particle Nom       = "ã¯"
case_particle NomStrong = "ã®ã“ã¨ãŒ"
case_particle Acc       = "ã‚’"
...
instance Symantics JA where
  john             = JA (make_np "ã‚¸ãƒ§ãƒ³ã•ã‚“")
  mary             = JA (make_np "ãƒ¡ãƒªã•ã‚“")
  like             = JA (make_tv "å¥½ãã " NomStrong Nom)
  own              = JA (make_tv "é£¼ã£ã¦ã„ã‚‹" Acc Nom)
  r2 (JA f) (JA x) = JA (f x)
  r1 (JA x) (JA f) = JA (f x)
